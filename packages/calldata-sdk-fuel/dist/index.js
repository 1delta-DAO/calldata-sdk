...
function addressInput(address) {
  return { Address: { bits: fuels.Address.fromAddressOrString(address).toB256() } };
}
function contractIdInput(contractId) {
  return { ContractId: { bits: fuels.Address.fromAddressOrString(contractId).toB256() } };
}
...
function getDexContracts(protocols) {
  return ___default.default.uniq(
    ___default.default.uniq(protocols).map((protocol) => {
      switch (protocol) {
        case DexProtocol.MIRA_STABLE:
        case DexProtocol.MIRA_VOLATILE:
...
function aggregateInputs(qts) {
  const ccys = ___default.default.uniq(qts.map((q2) => q2.assetId));
  let q = [];
  ccys.map((assetId) => {
...
((CurrencyUtils2) => {
  ...
  function isNative(c) {
    return c.address === viem.zeroAddress;
  }
  CurrencyUtils2.isNative = isNative;
  function isNativeAmount(c) {
    return c.currency.address === viem.zeroAddress;
  }
...
function getEncodedFuelPathExactInFromTrade(trade, receiver, slippageToleranceBps, deadline) {
  ...
  const abiInterface = new fuels.Interface(batch_swap_exact_in_script_loader_abi_default);
...
function getEncodedFuelPathExactOutFromTrade(trade, receiver, slippageToleranceBps, deadline) {
  ...
  const abiInterface = new fuels.Interface(batch_swap_exact_out_script_loader_abi_default);
  const functionName = "main" /* Main */;
  return {
    params: abiInterface.getFunction(functionName).encodeArguments([path, deadline]),
    inputAssets,
...
exports.BPS_BASE = BPS_BASE;
exports.DEAD_LOGGER = DEAD_LOGGER;
exports.DIESEL_AMM_ID = DIESEL_AMM_ID;
exports.FuelPathConverter = FuelPathConverter;
exports.MIRA_AMM_ID = MIRA_AMM_ID;
exports.MIRA_HOOKS_ID = MIRA_HOOKS_ID;
exports.ScriptFunctions = ScriptFunctions;
